...
def test_update_top_tracks(update_top_tracks_manager: UpdateTopTracksManager, tracks):
    logger.info("[START] test_update_top_tracks")

    top_tracks = update_top_tracks_manager.run()
    assert len(top_tracks) == 10

    logger.info("[END] test_update_top_tracks")


def test_update_top_tracks_error_last_fm(update_top_tracks_manager: UpdateTopTracksManager):
...
def test_update_top_tracks_error_spotify(update_top_tracks_manager: UpdateTopTracksManager):
